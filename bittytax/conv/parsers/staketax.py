...
STAKETAX_MAPPING = {'STAKING': TransactionOutRecord.TYPE_STAKING,
...
def parse_staketax_default(data_row, parser, **_kwargs):
    ...
    if row_dict['tx_type'] == "TRANSFER":
        if buy_quantity and not sell_quantity:
            ...
            raise UnexpectedTypeError(parser.in_header.index('tx_type'), 'tx_type',
                                      row_dict['tx_type'])
    ...
    data_row.t_record = TransactionOutRecord(t_type,
                                             ...
                                             wallet=get_wallet(row_dict['exchange'],
                                                               row_dict['wallet_address']),
                                             note=row_dict['comment'])

    parser.worksheet_name = "StakeTax " + row_dict['exchange'].replace('_blockchain',
                                                                       '').capitalize()

def get_wallet(exchange, wallet_address):
...
DataParser(DataParser.TYPE_ACCOUNTING,
           "StakeTax",
           ['timestamp', 'tx_type', 'received_amount', 'received_currency', 'sent_amount',
            'sent_currency', 'fee', 'fee_currency', 'comment', 'txid', 'url', 'exchange',
            'wallet_address'],
...
DataParser(DataParser.TYPE_GENERIC,
           "StakeTax",
           ['Type', 'Buy Quantity', 'Buy Asset', 'Buy Value', 'Sell Quantity', 'Sell Asset',
            'Sell Value', 'Fee Quantity', 'Fee Asset', 'Fee Value', 'Wallet', 'Timestamp',
            'Note', 'Tx ID', 'URL', 'Raw Data'],
           worksheet_name="StakeTax",
