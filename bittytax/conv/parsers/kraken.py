...
QUOTE_ASSETS = ['AED', 'AUD', 'CAD', 'CHF', 'DAI', 'DOT', 'ETH', 'EUR', 'GBP', 'JPY', 'USD',
...
def parse_kraken_ledgers(data_rows, parser, **_kwargs):
    ...
    for row_index, data_row in enumerate(data_rows):
        ...
        try:
            parse_kraken_ledgers_row(ref_ids, data_rows, parser, data_row, row_index)
        except DataRowError as e:
            data_row.failure = e

def parse_kraken_ledgers_row(ref_ids, data_rows, parser, data_row, row_index):
    ...
    if row_dict['type'] == "deposit":
        if Decimal(row_dict['amount']) > 0:
            ...
            if Decimal(row_dict['fee']) < 0:
                ...
                dup_data_row.row = []
                dup_data_row.t_record = TxOutRec(TxOutRec.TYPE_GIFT_RECEIVED,
                                                 data_row.timestamp,
        ...
        if row_dict['subtype'] == "":
            if Decimal(row_dict['amount']) >= 0:
                ...
                # Delisting
                data_row.t_record = TxOutRec(TxOutRec.TYPE_LOST,
                                             data_row.timestamp,
                                             sell_quantity=abs(Decimal(row_dict['amount'])),
                                             sell_asset=normalise_asset(row_dict['asset']),
                                             wallet=WALLET)
        elif row_dict['subtype'] in ("spottostaking",
                                     "stakingtospot",
                                     "spotfromstaking",
                                     "stakingfromspot"):
...
def make_trade(ref_ids):
    ...
    for data_row in ref_ids:
        ...
        if Decimal(row_dict['amount']) == 0:
            ...
            data_row.t_record = TxOutRec(TxOutRec.TYPE_SPEND,
                                         data_row.timestamp,
        ...
        if Decimal(row_dict['fee']) > 0:
            if not fee_quantity:
                ...
                data_row.t_record = TxOutRec(TxOutRec.TYPE_SPEND,
                                             ...
                                             note="Trading fee")
    if trade_row:
        trade_row.t_record = TxOutRec(TxOutRec.TYPE_TRADE,
                                      trade_row.timestamp,
