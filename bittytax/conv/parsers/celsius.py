...
def parse_celsius(data_row, parser, **kwargs):
    ...
    if row_dict['Transaction type'] in ("deposit", "Deposit",
        ...
        data_row.t_record = TxOutRec(TxOutRec.TYPE_DEPOSIT,
                                     ...
                                     wallet=WALLET)
    elif row_dict['Transaction type'] in ("withdrawal", "Withdrawal",
        ...
        data_row.t_record = TxOutRec(TxOutRec.TYPE_WITHDRAWAL,
                                     ...
                                     wallet=WALLET)
    elif row_dict['Transaction type'] in ("interest", "Interest",
                                          "reward", "Reward"):
        data_row.t_record = TxOutRec(TxOutRec.TYPE_INTEREST,
                                     data_row.timestamp,
                                     buy_quantity=row_dict['Coin amount'],
                                     buy_asset=row_dict['Coin type'],
                                     buy_value=DataParser.convert_currency(
                                         row_dict['USD Value'], 'USD',
                                         data_row.timestamp),
                                     wallet=WALLET)
    elif row_dict['Transaction type'] in ("promo_code_reward", "Promo Code Reward",
                                          "referred_award", "Referred Award",
                                          "referrer_award", "Referrer Award",
