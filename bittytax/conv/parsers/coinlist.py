...
def get_buy_quantity(description):
...
def parse_coinlist_pro(data_rows, parser, **_kwargs):
    ...
    for data_row in data_rows:
        ...
        if row_dict['type'] in ("match", "fee"):
            buy, sell = get_buy_sell(data_row, "match", tx_times[row_dict['time']])
            fee = get_fee(data_row, tx_times[row_dict['time']])
            if buy is None or fee is None:
                data_row.failure = MissingComponentError(parser.in_header.index('time'), 'time',
                                                         row_dict['time'])
                continue
            ...
            data_row.failure = UnexpectedTypeError(parser.in_header.index('type'), 'type',
                                                   row_dict['type'])

def get_buy_sell(data_row, tx_type, tx_times):
