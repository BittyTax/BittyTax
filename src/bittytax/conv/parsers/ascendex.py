...
def parse_ascendex_trades_statement(
    data_row: "DataRow", parser: DataParser, **_kwargs: Unpack[ParserArgs]
) -> None:
    row_dict = data_row.row_dict
    data_row.timestamp = DataParser.parse_timestamp(row_dict["Date(UTC)"])
...
def parse_ascendex_deposits_withdrawals_crypto_v2(
    data_row: "DataRow", parser: DataParser, **_kwargs: Unpack[ParserArgs]
) -> None:
    row_dict = data_row.row_dict
...
def parse_ascendex_deposits_withdrawals_crypto_v1(
...
def parse_ascendex_deposits_withdrawals_cash(
    data_row: "DataRow", parser: DataParser, **kwargs: Unpack[ParserArgs]
) -> None:
    row_dict = data_row.row_dict
...
def parse_ascendex_statements(
    data_rows: List["DataRow"], parser: DataParser, **_kwargs: Unpack[ParserArgs]
) -> None:
    tx_times: Dict[datetime, List["DataRow"]] = {}

    for dr in data_rows:
        dr.timestamp = DataParser.parse_timestamp(dr.row_dict["Datetime"])
    ...
    for data_row in data_rows:
        ...
        try:
            _parse_ascendex_statements_row(tx_times, parser, data_row)
...
def _parse_ascendex_statements_row(
    ...
    if row_dict["Account"] in ("USDT-Futures", "USD-MFutures", "USD-M Futures", "Coin-M Futures"):
        _parse_ascendex_statements_futures_row(tx_times, parser, data_row)
        return

 #   if row_dict["Account"] in ("Isolated Margin", "CrossMargin", "Cross Margin"):
 #       _parse_ascendex_statements_margin_row(tx_times, parser, data_row)
 #       return
    ...
    if row_dict["Operation"] in ("deposit", "Deposit", "Fiat Deposit"):
        data_row.t_record = TransactionOutRecord(
            TrType.DEPOSIT,
            data_row.timestamp,
            buy_quantity=Decimal(row_dict["Change"]),
    ...
    elif row_dict["Operation"] in ("withdrawal", "Withdraw", "Fiat Withdraw"):
        data_row.t_record = TransactionOutRecord(
            TrType.WITHDRAWAL,
            data_row.timestamp,
            sell_quantity=abs(Decimal(row_dict["Change"])),
...
def _parse_ascendex_statements_futures_row(
    tx_times: Dict[datetime, List["DataRow"]], parser: DataParser, data_row: "DataRow"
) -> None:
    row_dict = data_row.row_dict

    if row_dict["Operation"] in ("realisedPNL",
        "Realize profit and loss", "Realized Profit and Loss"):
        if Decimal(row_dict["Change"]) > 0:
            data_row.t_record = TransactionOutRecord(
                TrType.MARGIN_GAIN,
                data_row.timestamp,
    ...
    elif row_dict["Operation"] in ("fee","Fee", "Insurance Fund Compensation"):
        data_row.t_record = TransactionOutRecord(
            TrType.MARGIN_FEE,
            data_row.timestamp,
            sell_quantity=abs(Decimal(row_dict["Change"])),
    ...
    elif row_dict["Operation"] in ("funding_fee", "Funding Fee"):
        if Decimal(row_dict["Change"]) > 0:
            data_row.t_record = TransactionOutRecord(
...
"""
def _parse_ascendex_statements_margin_row(
    tx_times: Dict[datetime, List["DataRow"]], parser: DataParser, data_row: "DataRow"
) -> None:
    row_dict = data_row.row_dict
...
def parse_ascendex_futures(
    ...
    for data_row in data_rows:
        ...
        try:
            _parse_ascendex_futures_row(tx_ids, parser, data_row)
        except DataRowError as e:
            data_row.failure = e
        except (ValueError, ArithmeticError) as e:
            if config.debug:
...
def _parse_ascendex_futures_row(
...
DataParser(
    ...
    worksheet_name="Ascendex T",
    row_handler=parse_ascendex_trades,
)
...
DataParser(
    ...
    worksheet_name="Ascendex T",
    row_handler=parse_ascendex_convert,
)
...
DataParser(
    ...
    worksheet_name="Ascendex T",
    row_handler=parse_ascendex_trades_statement,
)
...
DataParser(
    ...
    worksheet_name="Ascendex D,W",
    row_handler=parse_ascendex_deposits_withdrawals_crypto_v2,
)

DataParser(
    ParserType.EXCHANGE,
    "Ascendex Withdrawals",
    ["Date(UTC+0)", "Coin", "Network", "Amount", "Fee", "Address", "TXID", "Status"],
    worksheet_name="Ascendex D,W",
    row_handler=parse_ascendex_deposits_withwithdrawals_crypto_v2,
)

DataParser(
    ParserType.EXCHANGE,
    "Ascendex Deposits/Withdrawals",
    [
        "Date(UTC)",
        "Coin",
        "Network",
    ...
    worksheet_name="Ascendex D,W",
    row_handler=parse_ascendex_deposits_withdrawals_crypto_v1,
)
...
DataParser(
    ...
    worksheet_name="Ascendex D,W",
    row_handler=parse_ascendex_deposits_withwithdrawals_crypto_v1,
)
...
DataParser(
    ...
    worksheet_name="Ascendex D,W",
    row_handler=parse_ascendex_deposits_withdrawals_cash,
)
...
DataParser(
    ...
    worksheet_name="Ascendex F",
    all_handler=parse_ascendex_futures,
)
...
DataParser(
    ParserType.EXCHANGE,
    "Ascendex Statements",
    ["Datetime", "Account", "Operation", "Coin", "Change", "Remark", "Ascendex"],
    worksheet_name="Ascendex S",
    all_handler=parse_ascendex_statements,
)
