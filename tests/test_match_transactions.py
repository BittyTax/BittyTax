...
from bittytax.tax_event import TaxEventCapitalGains, TaxEventNoGainNoLoss
...
config.config["cost_tracking_method"] = CostTrackingMethod.UNIVERSAL
# config.config["universal_ordering"] = UniversalOrdering.TID
config.config["universal_ordering"] = UniversalOrdering.DATE
...
def add_swap(
    ...
    swap.parse()
    assert swap.t_record
    return swap.t_record
...
def test_fifo_1() -> None:
    ...
    assert isinstance(tax_events[0], TaxEventCapitalGains)
    ...
    z_buy1 = make_buy(1.5, "BTC", 0, "Main", sell1.timestamp, Tid((2, 5, 0)))
    assert [str(b) for b in tax_events[0].buys] == [str(buy1), str(z_buy1)]

    assert isinstance(tax_events[1], TaxEventCapitalGains)
    ...
    assert isinstance(tax_events[2], TaxEventCapitalGains)
    ...
    assert [str(b) for b in tax.buy_list[AssetSymbol("BTC")].all()] == [
        str(buy4),
        str(buy1),
        str(z_buy1),
        str(buy2),
...
def test_fifo_2() -> None:
    ...
    assert tax_events[2].proceeds == 70000
    assert str(tax_events[2].sell) == str(sell2)
    s_buy2_1 = make_buy(1, "BTC", 50000, buy2.wallet, buy2.timestamp, Tid((4, 1, 0)))
    s_buy2_2 = make_buy(3, "BTC", 150000, buy2.wallet, buy2.timestamp, Tid((4, 1, 1)))
...
def test_fifo_swap_1() -> None:
    tr_swap1 = add_swap(19, "DOT", 1900, "DOT", "Main", "2020-08-21 12:00:00")
    transactions = get_transactions([tr_swap1])

    # Keep copy of buys/sells to detect any changes
    buy1 = copy.deepcopy(tr_swap1.buy)
    sell1 = copy.deepcopy(tr_swap1.sell)
    assert buy1
    assert sell1
    ...
    assert isinstance(tax_events[0], TaxEventNoGainNoLoss)
    assert tax_events[0].disposal_type is DisposalType.SHORT_TERM
    assert tax_events[0].cost == 0
    assert tax_events[0].market_value == 0
    assert str(tax_events[0].sell) == str(sell1)
    z_buy1 = make_buy(1900, "DOT", 0, "Main", sell1.timestamp, Tid((0, 4, 0)))
    assert [str(b) for b in tax_events[0].buys] == [str(z_buy1)]

    assert tr_swap1.buy and tr_swap1.buy.matched is False
    assert tr_swap1.sell and tr_swap1.sell.matched is True

    assert [str(b) for b in tax.buy_list[AssetSymbol("DOT")].all()] == [
        str(z_buy1),
        str(buy1),
    ]
...
def test_hifo_1() -> None:
    ...
    s_buy1_1 = make_buy(1, "BTC", 2475, buy1.wallet, buy1.timestamp, fee=25, tid=Tid((1, 1, 0)))
    s_buy1_2 = make_buy(1, "BTC", 2475, buy1.wallet, buy1.timestamp, fee=25, tid=Tid((1, 1, 1)))
...
def test_hifo_2() -> None:
    ...
    assert tax_events[1].proceeds == 6000
    assert str(tax_events[1].sell) == str(sell2)
    s_buy1_1 = make_buy(1, "BTC", 2500, buy1.wallet, buy1.timestamp, Tid((1, 1, 0)))
    s_buy1_2 = make_buy(1, "BTC", 2500, buy1.wallet, buy1.timestamp, Tid((1, 1, 1)))
    assert [str(b) for b in tax_events[1].buys] == [str(s_buy1_1)]
...
def test_lofo_1() -> None:
    ...
    assert tax_events[1].proceeds == 6000
    assert str(tax_events[1].sell) == str(sell2)
    s_buy1_1 = make_buy(1, "BTC", 2500, buy1.wallet, buy1.timestamp, tid=Tid((1, 1, 0)))
    s_buy1_2 = make_buy(1, "BTC", 2500, buy1.wallet, buy1.timestamp, tid=Tid((1, 1, 1)))
    assert [str(b) for b in tax_events[1].buys] == [str(s_buy1_1)]
